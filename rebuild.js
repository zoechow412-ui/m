...
const liurenCalc=p=>{
  ...
  try {const lunar=window.Solar.fromYmdHms(p.year,p.month,p.day,p.hour,p.minute,0).getLunar(),month=Math.abs(lunar.getMonth()),day=lunar.getDay(),hour=Math.floor((p.hour+1)/2)%12+1,names=['大安','留連','速喜','赤口','小吉','空亡'],m=(month-1)%6,d=(m+day-1)%6,h=(d+hour-1)%6;return {month,day,hour,names,positions:[m,d,h],god:names[h],trace:`大安起正月：第 ${month} 月落 ${names[m]} → 月上起日：第 ${day} 日落 ${names[d]} → 日上起時：第 ${hour} 個時辰落 ${names[h]}`};}catch(e){return {error:`掐指一算失敗：${e.message}`};}
};
...
const buildReading=(id,r,p)=>{
  ...
  if(id==='liuren'){
    ...
    return baseLead(`掐指一算｜${r.god}`,`${term('小六壬','以農曆月、日、時辰在六個固定落宮順序推算；唔用起卦數字。','liu-method')}：${esc(r.trace)}`,`我手指最後停喺「${esc(r.god)}」。呢個唔係把六個答案一次過倒比你，而係先由你輸入嘅香港起課時間算出唯一落宮：${esc(meaning[r.god])}`)+section(`${term('落宮','最後由時辰推到嘅六神位置，是今次問題嘅當下課象。','liu-palace')}｜白話`,`<p>${esc(meaning[r.god])}。計算順序固定為：${esc(r.names.join(' → '))}；月 ${r.month}、日 ${r.day}、時辰數 ${r.hour}。六神係判讀語言，唔係六個獨立預言。</p>`)+section('工作／財運／感情／家庭等影響',`<div class="impact-grid">${impacts(focus,`落宮 ${r.god}；${meaning[r.god]}`)}</div>`)+section('可能時間窗口',`<p>傳統小六壬只提供短課節奏，今次以起課後 ${esc(dateWindow(r.god==='速喜'?9:r.god==='留連'?30:21))} 作驗證窗口；期間記錄實際訊息、延誤或衝突，唔將象意當成已發生事實。</p>`)+section('墨川要你而家做',`<p class="action-line">${r.god==='赤口'?'今日先唔好用情緒回覆重要訊息，隔一晚再核對事實。':r.god==='空亡'?'先補一項缺失資料，再決定要唔要行動。':r.god==='速喜'?'如果有窗口，先做一個小而不可逆風險低嘅行動。':'保留彈性，先完成一個基本步驟，等下一個實際訊號。'}</p>`);
  }
  if(id==='qimen'){
