...
const term = (label, explanation, key = label) => { state.terms.set(key, {label, explanation:termDetail(label, explanation)}); return `<button class="term-link" type="button" data-term="${esc(key)}">${esc(label)}</button>`; };
const glossary={
...
const termRole={
  '四柱':'把出生年、月、日、時分成四組，先確定排盤底座，再按日主與月令看五行互動。','日主':'將你本人放在盤中心，其他干支都要問「對日主係生、剋、洩、耗定同類」。','月令':'先定季節氣勢及旺衰底色，再校驗其他干支有冇根、有冇通關。','十神':'把同一組五行關係翻譯成資源、輸出、責任、同伴及財務等事件角色。','刑沖合害':'指出地支之間係靠近、拉扯、碰撞定暗耗；必須放返全盤先判輕重。','命宮':'定位紫微盤中「我點樣做選擇」嘅核心，不能代替官祿、財帛及對宮。','身宮':'補充你實際將時間與力氣投放喺邊，與命宮比較先有判讀價值。','十二宮':'將人生分成十二個生活場域，逐宮睇星曜，再用對宮及三方四正連結。','三方四正':'將一宮同三個會照位置放埋一齊，避免只見單一星曜就落結論。','四化':'追蹤資源、權責、名聲及卡點由邊個宮位流向另一個宮位。','Lagna':'用出生時間地點計出上升點，係印度占星將星體放入生活宮位嘅起點。','Bhava':'把行星功能落到工作、收入、關係、家庭等具體生活範圍。','Nakshatra':'用月亮所在月宿細分情緒模式及事件節奏，唔係另一個星座名稱。','Dasha':'將出生盤切成行星主導時段，用來排長期窗口，仍要對照實際事件。','Transit':'比較現時天空同出生盤嘅位置，補充近期較容易被觸發嘅主題。','本卦':'梅花／易經中代表目前局面嘅第一層結構，先問現況係點形成。','體用':'梅花用體代表主體、用代表外部或所問之事，再看五行生剋係支援定消耗。','互卦':'取本卦中段爻位，讀出表面結果底下嘅中途過程、內情及牽連。','動爻':'指出六爻中最先變化嘅位置，係目前最值得驗證嘅環節。','變卦':'把動爻翻轉後所得嘅方向，讀成行動後可能轉向，唔係保證結局。','小六壬':'以當刻農曆月、日、時辰順推六神，係短課，不用出生日期代替。','落宮':'小六壬最後停留嘅六神位置，集中描述今次問題嘅短期氣氛。','局數':'奇門按節氣、陰陽遁及時辰定出九宮框架，先確定局再讀門星神。','九宮':'將空間與事件分成九格，讓天盤、地盤、八門、九星、八神有落點。','八門':'把事情分成開展、休整、資源、阻力、隱藏、表現、停滯及驚動等入口。','九星':'描述策略傾向及處理方式，要與門、神及落宮一齊讀。','八神':'補充事情當刻嘅氣氛、助力、遮蔽或干擾，不能單獨判吉凶。','值符':'找出九星系統中局勢最有主導力嘅位置，反映核心權責。','值使':'找出八門系統中最直接嘅落地入口，反映下一步點做。','交叉核對':'比較不同術數係咪重複指向同一個問題層；相反結果本身亦係需要補證據嘅訊號。','時間窗口':'將象意轉為可以觀察及覆盤嘅日期範圍，唔係預告某日必然發生。'
};
const termDetail=(label,explanation)=>{const role=termRole[label]||`${label}要連同所在宮位、卦位、星曜及問題一齊讀，唔可以脫離盤面單獨下結論。`;const impact=label==='日主'||label==='十神'?'工作睇責任與輸出、財運睇資源及現金流、感情睇付出與界線、家庭睇角色分工。':label==='命宮'||label==='身宮'||label==='十二宮'||label==='三方四正'||label==='四化'?'工作睇官祿與權責、財運睇財帛與資源、感情睇夫妻與福德、家庭睇田宅與父母。':label==='Lagna'||label==='Bhava'||label==='Nakshatra'||label==='Dasha'||label==='Transit'?'工作睇行動入口、財運睇價值及風險、感情睇互動安全感、家庭睇生活根基。':label==='本卦'||label==='體用'||label==='互卦'||label==='動爻'||label==='變卦'?'工作、財運、感情、家庭都要先分清主體與外部條件，再看邊個環節先變。':label==='小六壬'||label==='落宮'?'工作、財運、感情、家庭只作短期節奏提醒，唔取代合約、對話、醫療或法律判斷。':'工作、財運、感情、家庭都只能作核對方向，真正影響要由你揀嘅主題及實際事件確認。';const windowText=label==='小六壬'||label==='落宮'||label==='動爻'?'以起課後 1–21 日作短期驗證，按訊息、延誤或轉折記錄。':label==='Dasha'||label==='大運'||label==='流年'?'先看目前年度／周期，再用實際事件校驗，不把周期當成指定日期。':'由今日香港時間起觀察未來 7–60 日，窗口係驗證期，不是必然預言。';const action=label==='本卦'||label==='互卦'||label==='變卦'?'先寫低限制、內在過程及可能轉向，各揀一件可撤回行動。':label==='小六壬'||label==='落宮'?'記低起課時間、實際回應及結果，下一次再比較，不要用出生資料重算。':label==='命宮'||label==='十二宮'||label==='三方四正'?'打開相關宮位及對宮，記一件七日內可以核對嘅事。':`把${label}指出嘅傾向轉成一個有期限、成本上限及回頭檢查點嘅行動。`;return `術語意思：${explanation}｜白話解釋：${role}｜盤面作用：${role}｜工作／財運／感情／家庭影響：${impact}｜可能時間窗口：${windowText}｜實際行動：${action}`;};
const registerGlossary=()=>Object.entries(glossary).forEach(([label,explanation])=>{if(!state.terms.has(label))state.terms.set(label,{label,explanation:termDetail(label,explanation)});});
...
const hkNowParts = () => Object.fromEntries(new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Hong_Kong',hour12:false,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}).formatToParts(new Date()).filter(x=>x.type!=='literal').map(x=>[x.type,Number(x.value)]));
...
const meihuaCalc=(p, q, mode='time')=>{
  ...
  try { const hk=hkNowParts(), lunar=window.Solar.fromYmdHms(hk.year,hk.month,hk.day,hk.hour,hk.minute,0).getLunar(), month=Math.abs(lunar.getMonth()), day=lunar.getDay(), yearBranch=((hk.year-4)%12+12)%12+1, hourIndex=Math.floor((hk.hour+1)/2)%12+1, total=yearBranch+month+day, solarDate=`${hk.year}-${String(hk.month).padStart(2,'0')}-${String(hk.day).padStart(2,'0')}`,solarTime=`${String(hk.hour).padStart(2,'0')}:${String(hk.minute).padStart(2,'0')}`; let upper,lower,moving,basis;
    if(mode==='question'){const seed=[...q].reduce((n,c)=>n+c.charCodeAt(0),hk.year+hk.month+hk.day+hk.hour+hk.minute); upper=(seed%8)||8;lower=((seed+hourIndex)%8)||8;moving=((seed+month+day)%6)||6;basis=`問題起卦：香港陽曆 ${solarDate} ${solarTime}；以「${q}」字序及當刻時間作起卦種子`;} else {upper=(total%8)||8;lower=((total+hourIndex)%8)||8;moving=((total+hourIndex)%6)||6;basis=`時間起卦：香港陽曆 ${solarDate} ${solarTime}；農曆 ${lunar.getYear()} 年 ${month} 月 ${day} 日；時辰數 ${hourIndex}`;}
    return {...hexBuild(upper,lower,moving,basis),solarDate,solarTime};
...
const qimenCalc=async(p,q)=>{
  try {const mod=await import('./public/qimen-engine.js'), hk=hkNowParts(), d=utcDate(hk,8), chart=mod.calculate(d,{type:'三元',method:'时家',location:p.place||'香港',purpose:q}); if(chart?.error) return {error:chart.message||'奇門排盤失敗'};return {chart,solarDate:`${hk.year}-${String(hk.month).padStart(2,'0')}-${String(hk.day).padStart(2,'0')}`,solarTime:`${String(hk.hour).padStart(2,'0')}:${String(hk.minute).padStart(2,'0')}`};}catch(e){return {error:`奇門排盤失敗：${e.message}`};}
};
...
const dateWindow=(days=21)=>{const a=new Date(Date.now()+86400000),b=new Date(Date.now()+days*86400000),fmt=d=>{const p=Object.fromEntries(new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Hong_Kong',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(d).filter(x=>x.type!=='literal').map(x=>[x.type,Number(x.value)]));return `${p.year} 年 ${p.month} 月 ${p.day} 日`;};return `${fmt(a)}至${fmt(b)}（香港時間觀察窗口，不是必然事件）`;};
...
const buildReading=(id,r,p)=>{
  ...
  if(id==='qimen'){
    const c=r.chart, raw=c.raw||{}, order=c.baseOrder||[], rows=`起局時間：香港陽曆 ${r.solarDate||'—'} ${r.solarTime||'—'}；`+order.map(g=>`${g}宮：地 ${raw.diPan?.[g]||'—'}／天 ${raw.tianPan?.[g]||'—'}／星 ${raw.jiuXing?.[g]||'—'}／門 ${raw.baMen?.[g]||'—'}／神 ${raw.baShen?.[g]||'—'}`).join('；');
...
init();
const ziweiDeepStar={
  '七殺':'主決斷、開創及高壓取捨。落命宮時常見先斬後奏、寧願自己扛責任；優點是危局敢行動，風險是太快切斷協商。',
  '紫微':'主統籌、主導及承擔。重視格局、名分與資源掌控；落福德時容易把「應該由我處理」變成內在壓力。',
  '天府':'主守成、收納及資源管理。要看庫存、現金流和長期安全感，不是單純代表有錢或穩定。',
  '廉貞':'主界線、規則、慾望及選擇。對模糊承諾較敏感，適合先定原則再談彈性。',
  '貪狼':'主吸引力、社交、多元興趣及欲望。機會多但容易分散，要核對承諾是否跟得上選項。',
  '破軍':'主拆舊立新、重整及變動。適合處理失效制度，行動前要設成本上限與退出線。',
  '天相':'主協調、公平、規格及接口。常替制度與人之間補位，要核對責任是否有相應權限。',
  '天機':'主分析、變通、計劃及訊息流動。能見到路線，但比較太多會令決定延遲。',
  '太陰':'主細節、內在安全感、累積及照顧。要看信任與資源是否逐步建立。',
  '太陽':'主公開、承擔、帶領及被看見。容易照顧別人，但責任、回報與權力要對等。',
  '武曲':'主執行、數字、效率及資源控制。適合管理與落地，關係上要補回情緒表達。',
  '天同':'主和氣、享受、適應及減壓。能柔化衝突，但面對取捨要設期限，避免拖延。',
  '巨門':'主言語、質疑、辨析及未說清楚的疑點。適合留證據、逐點問清，避免把猜測變指控。',
  '天梁':'主保護、原則、長輩及處理問題。容易扮演救火者，要分清幫忙與代人承擔。',
  '左輔':'主協助、配合及把計劃補完整。代表可借力，但要明確分工，不能只等別人主動。',
  '右弼':'主支援、協調及人情網絡。適合找可靠夥伴，並把口頭承諾轉成可追蹤安排。',
  '文昌':'主文字、學習、整理及表達。文件、紀錄與清楚說明會直接影響結果。',
  '文曲':'主感受、審美、表達技巧及迂迴理解。訊息太含蓄時容易被誤解，要補清楚。',
  '天魁':'主被提攜、關鍵人物及正規機會。要主動準備成果，機會才不會停在介紹層。',
  '天鉞':'主補位、轉介及危急時協助。適合接受有條件支援，同時保留自己判斷。',
  '天馬':'主移動、轉換、外務及速度。流動帶來機會，但未穩定就頻繁轉向會耗損。',
  '擎羊':'主直接阻力、切割及尖銳衝突。要用清楚界線和安全緩衝處理，不宜硬碰硬。',
  '陀羅':'主拖延、反覆、慢性阻力及卡在細節。把目標拆成小檢查點，才看得見進度。',
  '地空':'主落空感、資源未落實及計劃留白。先驗證承諾、款項與實物，不要按想像投入。',
  '地劫':'主突發損耗、被抽走及不可控缺口。要預留備用方案和停損線。',
  '旬空':'主暫時未落地、資訊空窗及承諾延後。先當待確認，不把沉默當接受或拒絕。',
  '孤辰':'主獨處、自理及難以即時求助。優點是自主，風險是把需要藏起來。',
  '寡宿':'主內在保留、慢熱及不容易交出脆弱。關係上要用明確溝通建立安全感。',
  '紅鸞':'主吸引、喜慶及關係觸發。有好感不等於承諾，仍要看雙方實際投入。',
  '咸池':'主魅力、社交及情感投射。要分清被吸引、被需要與真正互相承擔。',
  '天刑':'主規範、界線、責任與壓力。適合先查規則和後果，不宜靠情緒挑戰底線。',
  '天哭':'主失望、情緒負擔及難以言明的傷感。先承認感受，再用具體對話代替自我消化。',
  '天虛':'主期待落差、想像與實況未接上。以可觀察行動核對，不要用承諾填補空白。',
  '封誥':'主資格、認可、文件及被正式記錄。把成果整理成證明，對工作或學習較有幫助。',
  '解神':'主拆解、化解及找到出口。先把問題分段，逐個處理，不必一次解決全部。',
  '天德':'主緩衝、善意及留有餘地。可用來修補關係，但仍要保留證據及界線。',
  '年解':'主年度內的轉圜與解套機會。代表有修正空間，不代表問題自動消失。',
  '鳳閣':'主審美、文字、呈現與被理解。除了說得好看，更要讓對方知道如何回應。',
  '華蓋':'主獨特、專注、精神性及與眾不同。適合深耕專長，也要保留交流。',
  '台輔':'主幕後協助、流程補強及把資源接上。先講清楚誰負責，支援才會落地。',
  '天福':'主享有、照顧及生活中的緩衝。要把好處轉成可持續安排，不只等待運氣。',
  '天廚':'主享受、照料及具體生活品質。適合改善日常，但要留意花費與責任分配。',
  '陰煞':'主隱性顧慮、未明講的情緒及背後因素。先核對資訊來源，不要靠猜測補空白。',
  '天才':'主學習速度、特殊技巧及臨場解法。把天分變成可交付成果，才可被看見。',
  '天月':'主慢性疲勞、照護及需要時間修復的議題。以作息和專業意見核對，不作診斷。',
  '月德':'主緩和、修補及留有善意。適合先降溫再協商，但不能取代界線。',
  '天官':'主制度、職責、正式渠道及被授權。循正規程序比私下猜測更有效。',
  '天壽':'主耐性、保養及長期維持。重點是持續習慣，不是一次衝刺。',
  '天巫':'主直覺、療癒及非理性體驗。可作內在感受參考，重要決定仍要回到事實。',
  '蜚廉':'主流言、速度、外部評價及訊息偏差。傳聞要先核實，避免即時反應。',
  '截路':'主中途受阻、路線被截及要改道。預先準備替代方案，唔好把一次阻滯當終局。',
  '天空':'主空白、延後及未成形的空間。先確認條件是否存在，再投入時間或金錢。',
  '破碎':'主拆散、細節破口及安排不完整。逐項補漏洞，避免只修表面。',
  '天使':'主制度要求、照顧責任及需要被提醒的地方。把責任寫清楚，避免由一個人默默承擔。'
};
document.addEventListener('click',e=>{const b=e.target.closest('.term-link');if(!b||state.method!=='ziwei')return;const label=b.textContent.trim(),detail=ziweiDeepStar[label];if(detail){setTimeout(()=>{$('#termBody').textContent=`術語意思：${detail}\n\n白話解釋：${label}唔係單獨吉凶符號，要放返落入「${state.topic}」相關宮位，再同主星、對宮及四化一齊睇。\n\n盤面作用：它補充該宮位的行事方式、阻力或資源，實際輕重要由落宮及會照核對。\n\n工作／財運／感情／家庭影響：工作睇責任與合作；財運睇資源與風險；感情睇互動與界線；家庭睇分工與支持。\n\n可能時間窗口：由今日香港時間起觀察未來 7–60 日，記錄是否有同一類事件重複出現。\n\n實際行動：把${label}指出的傾向轉成一個七日內、成本可控、可以覆盤的小行動。`;},0);}});
